...
int main( int argc, char **argv )
{
  if (argc != 14)
    {
      cerr << "sweep_der_randomX: sweeps in a derived population, following the 2-population model of Thornton and Jensen (2007) Genetics.  Trajectories are deterministic, and the position of the selected site is randomly-placed along the sampled region.\n"
	   << "usage: sweep_der nsam_ancestral nsam_derived nreps t_r d f N 2Ns tau rho_site nsites theta_site seed\n"
	   << "where\n"
  ...
  for(int rep = 0 ; rep < nreps ; ++rep)
    {
      //pick a random position X from 0 <= X < nsites
