...
int main( int argc, char **argv )
{
  if(argc != 10)
    {
      cerr << "rsweep: recurrent hitch-hiking at linked sites(i.e. Braverman 1995), with deterministic trajectories\n"
	   << "usage: rsweep nsam nreps N s lambda rho_locus nsites theta_locus seed\n"
	   << "where:\n"
...
//const double s = alpha/(2.*N);
  ...
  //const int k = 2;
  //const double dtp = 1./double(k*N),ifreq=1./double(2*N);
  //unsigned nsweeps = 0;
	  ...
	  if( neutral )
	      ...
	      else
		{
	      ...
	      selective_phase(uni,uni01,sample,sample_history,nsam,&NSAM,
			      &nlinks,&t,rho,nsites,N,path,X,1./double(2*N),1./double(4*N));
	      neutral = true;
	    }
	}
