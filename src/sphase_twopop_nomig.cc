...
{
  const double oldt=*t;
  const double dt = 1./(4.*double(k)*floor(double(N))),
    pmin = 1./(2.*floor(double(N)));
  ...
  while(path[selgen]>=pmin)
    {
      //cerr << *t << ' ' <<selgen<<' '<< path[selgen] << ' ' << config[FAVORED] << ' ' << config[UNFAVORED] << ' ' << config[ANCESTRAL] << '\n';
      double pr=1.;
      double r,sum=0.;
      int nevents=0,jump=0;
      while( (r=uni01())== 1. );
      selgen = int(tsel*4.*double(k)*double(N));
      //cerr << selgen << '\n';
      ...
      while( (pr>r) && (path[selgen]>=pmin) )
	{
...
#ifndef NDEBUG
	  if ( !isfinite(pevents[UNFAVCOAL]) )
	    {
	      cerr << config[UNFAVORED] << ' ' << dt
		   << ' ' << dt*double(config[UNFAVORED]*(config[UNFAVORED]-1)) <<' '
		   << 1.-path[selgen] << '\n';
	    }
	  
	  assert(isfinite(pevents[UNFAVCOAL]));
#endif
	  ...
	  pevents[ANCCOAL] = (config[ANCESTRAL]>1) ? dt*double(config[ANCESTRAL]*(config[ANCESTRAL]-1)) : 0.;
	  pevents[ANCREC] = (rho>0.&&config[ANCESTRAL]) ? dt*rho*double(nlinks_sel[ANCESTRAL]) : 0.;
	  ...
	  if(!isfinite(sum))
		...
		{
		  cerr << SEL_EVENT(i) << ' ' << pevents[i] << '\n';
		}
	      cerr << pevents[UNFAVCOAL] << '\n';
	    }
      ...
      if(sum>0.)
	{
	  ...
	  if ((*t+ttemp) >= tmerge && !*merged)//merge all of population 0 into pop 1 and 2
	    {
	      *t = tmerge;
	      //cerr << "at merge, changing tsel from " << tsel << " to ";
	      tsel = *t-oldt;
	      //cerr << tsel << '\n';
	      for( int i = 0 ; i < *NSAM ; ++i )
		{
		  if( sample[i].pop == ANCESTRAL )
		    {
		      int newpop = (uni01() <= path[selgen]) ?FAVORED:UNFAVORED;
		      sample[i].pop = newpop;
		      config[ANCESTRAL]--;
		      config[newpop]++;
		      nlinks_sel[ANCESTRAL] -= sample[i].links();
		      nlinks_sel[newpop] += slinks(sample[i],X);
		    }
		}
	      *merged = true;
	    }
